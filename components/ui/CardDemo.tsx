...
export function CardDemo({ title, description, icons }: CardDemoProps) {
  return (
    <Card>
...
const Skeleton = ({ icons }: SkeletonProps) => {
  const scale = [1, 1.1, 1];
  ...
  return (
    <div className="p-8 overflow-hidden h-full relative flex items-center justify-center">
      <div className="flex flex-row shrink-0 justify-center items-center gap-2">
          ...
          <Container key={index} className={`h-10 w-10 circle-${index + 1}`}>
            {icon}
          </Container>
        ))}
      </div>
